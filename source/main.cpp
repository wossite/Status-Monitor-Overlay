    ...
    virtual tsl::elm::Element* createUI() override {
		...
		auto Status = new tsl::elm::CustomDrawer([](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
			...
			if (!GameRunning) {
				uint32_t size = 18;
				uint32_t offset1 = 0;
				renderer->drawRect(0, 0, tsl::cfg::FramebufferWidth - 150, 22, a(0x7111));
				renderer->drawString("RAM", false, offset1, size, size, renderer->a(0xFCCF));
				renderer->drawString(RAM_var_compressed_c, false, offset1+47, size, size, renderer->a(0xFFFF));
			}
			else {
				uint32_t size = 18;
				uint32_t offset1 = 0;
